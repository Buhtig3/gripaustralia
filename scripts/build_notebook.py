...
and generate the companion Quarto report (gripsport_analysis.qmd) styled to
Calibrated Athletic Minimalism design tokens.
"""
import sys
import asyncio
...
# Suppress ProactorEventLoop warning on Windows for zmq
...
// Calibrated Theme Color Tokens matching global.css
$body-bg: #f8f9fa;
$body-color: #0f172a;
$card-bg: #ffffff;
$primary: #143024;
...
// DataTables and HTML Tables styling matching .prose-grip
.dataTables_wrapper, table.dataTable {
  background-color: #ffffff;
  border-radius: 8px;
  border: 1px solid #e2e8f0;
  box-shadow: 0 4px 14px rgba(20, 48, 36, 0.05);
  margin: 1.25rem 0;
...
.callout-note {
  border-left-color: #143024 !important;
  border-radius: 8px;
  background-color: #ffffff;
  box-shadow: 0 4px 12px rgba(20, 48, 36, 0.04);
}

.callout-tip {
  border-left-color: #e5a93c !important;
  border-radius: 8px;
  background-color: #ffffff;
  box-shadow: 0 4px 12px rgba(20, 48, 36, 0.04);
}
...
def add_md(text: str):
    cells_data.append(("markdown", text.strip()))

def add_code(code: str):
    cells_data.append(("code", code.strip()))


# --- Intro ---
add_md(r"""# 🏋️‍♂️ GripSport Analytics: Athlete Performance & Leaderboards
...
# --- Setup Cell ---
add_code(r"""# Environment & Calibrated Theming Setup
import duckdb
import pandas as pd
import numpy as np
...
# --- Data Ingestion & Bayesian Smoothing ---
add_md(r"""## 1. Data Ingestion & Integration
...
add_code(r"""# High-level aggregate statistics: All Events vs. Tracked Events
...
add_code(r"""# Interactive Scatter Plot: All Events vs Tracked Events (Plotly)
...
add_code(r"""# Static Visualizations: Scatter with Annotations & Untracked Win ECDF Plot
...
add_md(r"""### 🏺 Athletes with Most Legacy / Untracked Wins
...
add_code(r"""# Legacy Leaders Table
...
add_code(r"""# Compute division rankings and rank shifts
...
add_code(r"""# Competitor Quadrant Visualization
...
add_md(r"""### Bayesian-Smoothed Leaderboard (Minimizing Sample Size Noise)
...
add_code(r"""top_bayes = df.sort_values(
...
add_code(r"""# Career Era Aggregates
...
add_code(r"""# Visualizing Era Differences in Standardization
...
add_code(r"""def categorize_implement(name):
    n = name.lower()
...
add_code(r"""# Men's Division Leaderboards
...
add_code(r"""# Women's Division Leaderboards
...
add_code(r"""# Visual Comparison: Top Men & Women (All vs Tracked Placements)
...
add_code(r"""def compute_gini(arr):
...
add_code(r"""# Visualizing National Depth vs Elite Peak (Scatter)
...
add_md(r"""### 🇦🇺 Australian Athletes Detailed Roster
Complete breakdown for Australia's qualifying competitors ($\ge 8$ contests or notable medalists):
""")

add_code(r"""aus_roster = df[df['country'] == 'Australia'].sort_values(
...
# -----------------------------------------------------------------------------
# Compilation: Generate gripsport_analysis.ipynb and gripsport_analysis.qmd
# -----------------------------------------------------------------------------
def build_all():
    # 1. Build Jupyter Notebook
